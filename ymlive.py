...
# Устойчивое получение состояния плеера (не падает при отсутствии player_state)
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            ...
            async with session.ws_connect(
                ...
                try:
                    ...
                    logging.warning("raw_get_current_track_ws: couldn't parse second ws response: %s", recv.data)
                    return {"success": False}
                ...
                # если нет текущего трека — вернуть paused информацию (если есть)
                if track_index == -1:
...
class YmLive(loader.Module):
    """YandexMusicLive — название канала = трек, 1 сообщение = автор, обложка -> аватарка"""

    strings = {
        "name": "YandexMusicLive",
        "_text_token": "Токен аккаунта Яндекс Музыки",
        "_text_id": "ID канала для показа треков (только цифры, без -100)",
        "_text_idle": "Путь к фото-заглушке, когда ничего не играет (будет создан при .setidlepic)",
    ...
    def __init__(self):
        ...
        # состояние последнего трека и момент изменения
        self._last_track_title = None
        self._last_change_ts = 0

    async def client_ready(self, client, db):
        # обычно Hikka передаёт Telethon client сюда
        self._client = client
        self.db = db
        # inline bot доступен через self.inline.bot, self.inline.bot_username задано автоматически

    async def _normalize_channel_peer(self):
        """
        Возвращает int peer для работы с Telethon и Inline API.
        Конфиг хранит ID канала обычно без -100 префикса (например: 123456789).
        Мы всегда возвращаем значение с -100 префиксом (например -100123456789).
        """
        raw = str(self.config["channel_id"])
        if raw.startswith("-100"):
            return int(raw)
        # удаляем возможные плюсы/пробелы
        raw = raw.strip().lstrip("+")
        try:
            return int(f"-100{raw}")
        except Exception:
            # если всё совсем плохо — попробуем int напрямую
            try:
                return int(raw)
            except Exception:
                return None
    ...
    async def add_bot_to_channel(self, channel_id):
        """Добавление бота в канал (попытка), не критично если упадёт"""
        try:
            await self._client(
                EditAdminRequest(
            ...
            logging.info("add_bot_to_channel: не удалось выдать права боту (возможно уже есть): %s", e)

    async def get_current_track(self, token):
        """
        Возвращает:
            None              - не удалось получить/ошибка (будет срабатывать таймер 10мин)
            {"paused": True}  - известно, что на паузе (поставим статус паузы)
            {"title":..., "artists":[...], "cover_uri":..., "paused": False} - нормальный трек
        """
        try:
            ...
            # track может быть списком или объектом
            ...
            logging.exception("get_current_track wrapper exception: %s", e)
            return None

    async def update_channel_title(self, peer, title):
        """Меняем title канала (через inline.bot чтобы оно отражалось как название канала)"""
        try:
            # inline.bot.set_chat_title принимает peer (int like -100123456)
    ...
    async def edit_or_send_one_message(self, peer, text):
        """Редактируем единственное сообщение в канале или отправляем новое и сохраняем ID"""
        try:
            saved_id = self.get("channel_msg_id")
            # пробуем редактировать сохранённое сообщение
            if saved_id:
                try:
                    await self._client.edit_message(peer, saved_id, text)
                    return
                except Exception:
                    # не получилось редактировать сохранённое — попытаемся получить последнее
                    logging.info("edit_or_send_one_message: saved id not editable, will try last message")
            # пытаемся редактировать последний пост
            try:
            ...
            # если не получилось редактировать — отправляем новое и сохраняем id
